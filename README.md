...
## Project Content
This project will create EKS cluster with t2.micro ec2-instances, asg, with new VPC, subnets in different AZs

We create the following infrastructure on AWS with the instructions given below.
- EC2 Instances
- EKS Cluster
- ASG with desired capacity of 2 and a min of 2 to max 3 instances

There are couple of ways we can deploy EKS cluster on AWS:
using native eksctl
using cloudformation
*** using terraform and using terraform to build in this case.
...
terraform apply or terraform apply --auto-approve with out waiting for prompts //final command to execute the provision of infra on cloud or on-premise
```` 
The deployment of infra will take around 10 - 15 min.
